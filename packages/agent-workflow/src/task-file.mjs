...
export function parseTask(text,root){
 ...
 const editable=new Set(files.map(file=>{
  const target=resolve(root,file);if(!inside(resolve(root),target))throw new Error(`Editable path escapes root: ${file}`);return normalize(file).replaceAll('\\','/');
 }));
