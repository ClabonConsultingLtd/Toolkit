...
At every resume, use `sync`, then inspect every noncompleted ticket's saved Paseo agent status/activity, workspace, and linked PR. Session idle is not proof of successful implementation. Surface pending permissions without approving them on the user's behalf. Record `block` for errors/permission requests, preserving an active slot until the worker is confirmed stopped. Restart a blocked worker through `resume`. If its saved agent was restarted externally, first call `attach` with `workerActive: true` to restore capacity accounting, then `resume` with evidence. Never retry blocked tickets automatically.
