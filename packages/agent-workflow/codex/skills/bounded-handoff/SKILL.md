...
Use the Toolkit agent-workflow package's `handoff` command; the provider is selected by environment configuration, not by this skill. This skill is self-contained so it still works when copied into a skills directory.
