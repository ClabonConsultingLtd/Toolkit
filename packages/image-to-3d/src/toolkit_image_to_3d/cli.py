...
def parse_record(item: Any, source_root: Path) -> tuple[str, Path, Path]:
    ...
    return (
        ...
        resolve_queue_path(values["model"], source_root),
    )
    ...
    *,
    ...
    try:
        ...
        return (
            ...
            0,
            0,
            "gradio-client is not installed; install the package dependencies",
        )
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            preprocessed = client.predict(
                input=handle_file(str(reference)), api_name="/preprocess_image"
            )
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "queue", type=Path, help="JSON queue containing id/reference/model records"
    )
    result.add_argument(
        "--source-root",
        type=Path,
        default=Path.cwd(),
        help="root for relative queue paths (default: current directory)",
    )
    result.add_argument(
        "--token-env",
        default="HF_TOKEN",
    ...
    result.add_argument(
        "--results", type=Path, help="per-run JSON report (default: beside queue)"
    )
    result.add_argument("--ledger", type=Path, help="optional cumulative JSON ledger")
    result.add_argument(
        "--overwrite", action="store_true", help="replace an existing model output"
    )
    result.add_argument(
        "--dry-run",
        action="store_true",
        help="validate and report actions without calling the service",
    )
    return result
...
def main() -> None:
    ...
    for identifier, reference, model in records:
        ...
        if not reference.is_file():
            ...
            ok, size, attempts, error = convert(
                reference,
                model,
                token=token or "",
                space=args.space,
        ...
        print(
            f"{status.upper()}: {identifier}" + (f" - {error}" if error else ""),
            flush=True,
        )
        if quota_error(error):
            print(
                "Stopping batch: service quota or rate limit is exhausted.", flush=True
            )
            break
